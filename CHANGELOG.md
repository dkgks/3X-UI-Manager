...
- **Encrypted Happ link in the client's share sheet** (panel 3.8.0). When the operator allows encrypted subscription links on the panel, the subscription section creates a `happ://crypt5/…` link with its QR code; otherwise it says where that switch is.
...
### Fixed
- **A link too long for a QR code could crash the client's share sheet.** The sheet now shows a note in place of the code, and copy and share keep working.
...
- **The empty IP log now explains itself** — the panel keeps an address for 30 minutes after the last connection, so the log is empty for anyone who has not connected recently; that is no longer indistinguishable from something being broken.
...
- **Export / import rules, outbounds and DNS servers.** Routing rules, outbounds
  and DNS servers can be exported to JSON (copy or share) and imported back by
  pasting JSON — a bare array, `{rules|outbounds|servers:[…]}`, or the wrapped
  `{routing|dns:…}` form; imported entries are appended.
